# settings — video/audio settings and their persistence

Confirmed in the Top Gear Rally ROM: **3 functions, 176 bytes (0.04% of the 457,392-byte `.text`)**, of which **0 are byte-exact**.

The source that produced these is shared with the PC decomp and lives in:

  - `src/core/settings/br_basedir.c`
  - `src/core/settings/br_save.c`
  - `src/core/settings/br_state.c`

N64-only code for this area, once recovered, belongs in this folder. Per-function detail is in `n64/config/functions_tgr.csv`.
