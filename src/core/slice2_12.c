...
/* The bit-stream encoder shifts at the QUANTISER'S OWN WIDTH, not at 32 bits:
 * `sar ax,8` / `sar al,3`, with the `movsx` only at the push.  Every quantiser
 * clamps into its narrow range before returning, so the narrowing cast is
 * value-preserving and these agree with BrSar on both builds.  Spilling the
 * result to a named narrow local before the call reproduces the original's
 * `sar <narrow>; movsx <wide>; push` ordering (proven variant g3, REGNORM
 * 33+0 -- the residual 33 is the WriteBits thiscall edx slot, issue below). */
#ifdef BR_MATCHING_BUILD
#define BrSar16(v, n)  ((short)(v) >> (n))
#define BrSar8(v, n)   ((signed char)(v) >> (n))
#else
#define BrSar16(v, n)  ((short)BrSar((int32_t)(short)(v), (n)))
#define BrSar8(v, n)   ((signed char)BrSar((int32_t)(signed char)(v), (n)))
#endif

/* `fcomp <mem>; fnstsw ax; test ah,0x40; jne` -- C3 is set for EQUAL and for
 * UNORDERED, so "non-zero" in the original excludes NaN.  The original inlines
 * this at every flag write (`fld; fcomp 0.0; fnstsw; test ah,0x40; jne; mov
 * reg,1`), so it must be __inline here or VC5 emits a call instead. */
#ifdef BR_MATCHING_BUILD
...
void BrCarStateEncode(BrBitStream *pBs, const BrCarState *pSrc)
{
    ...
    /* Orientation: s16 at Q15 (negative scale), keep the high byte.  The
     * shift happens at the quantiser's OWN width (`sar ax,8`) and is widened
     * only at the push (`movsx ecx,ax`), so each value lives in a narrow
     * local rather than in an int temporary. */
    s = BrSar16(BrFixPackS16Q15Neg(pSrc->f00), 8);
    BrBitStreamWriteBits(pBs, s, 8);
    s = BrSar16(BrFixPackS16Q15Neg(pSrc->f04), 8);
    BrBitStreamWriteBits(pBs, s, 8);
    s = BrSar16(BrFixPackS16Q15Neg(pSrc->f08), 8);
    BrBitStreamWriteBits(pBs, s, 8);
    s = BrSar16(BrFixPackS16Q15Neg(pSrc->f0C), 8);
    BrBitStreamWriteBits(pBs, s, 8);
    ...
    s = BrSar16(BrFixPackS16Q7(pSrc->f18), 1);
    BrBitStreamWriteBits(pBs, s, 15);

    BrBitStreamWriteBits(pBs, (short)BrFixPackS16Q8(pSrc->f1C), 16);
    BrBitStreamWriteBits(pBs, (short)BrFixPackS16Q8(pSrc->f20), 16);

    c = BrSar8(BrFixPackS8Q3(pSrc->f28), 3);
    BrBitStreamWriteBits(pBs, c, 5);
    c = BrSar8(BrFixPackS8Q3(pSrc->f2C), 3);
    BrBitStreamWriteBits(pBs, c, 5);
    c = BrSar8(BrFixPackS8Q3(pSrc->f30), 3);
    BrBitStreamWriteBits(pBs, c, 5);
    c = BrSar8(BrFixPackS8Q3(pSrc->f34), 4);
    BrBitStreamWriteBits(pBs, c, 4);
    c = BrSar8(BrFixPackS6Q7Neg(pSrc->f38), 2);
    BrBitStreamWriteBits(pBs, c, 4);
