        ...
        )
{
        ...
        /* round newsize up the nearest paragraph. round 0 up to one
         * paragraph */
        if ( newsize > 0 )
            newsize = (newsize + _PARASIZE - 1) & ~(_PARASIZE - 1);
        else
            newsize = _PARASIZE;
        ...
        pmap = __sbh_find_block(pblock, &preg, &ppage);

        /* allocation block lies within the small-block heap, try to resize
         * it there. */
        if ( pmap != NULL ) {
            /* *pblock lies within the small-block, try to resize it there */
            retp = NULL;
            if ( (newsize <= __sbh_threshold) &&
                 __sbh_resize_block(preg, ppage, pmap, newsize >> _PARASHIFT) )
                retp = pblock;
            _munlock(_HEAP_LOCK);
            return retp;
        }
