...
/*      get a copy of the current day names */
char *_Getdays()
        {const struct _lc_time_data *pt = _lc_time_curr;
        size_t n, len = 0;
        char *p;
...
/*      get a copy of the current time locale information */
void *_Gettnames()
        {void *p = malloc(sizeof (struct _lc_time_data));
        if (p != 0)
                memcpy(p, _lc_time_curr, sizeof (struct _lc_time_data));
        return (p); }
...
struct __lc_time_data *lc_time = lc_time_arg == 0 ? _lc_time_curr
        : (struct __lc_time_data *)lc_time_arg;
size_t left;                    /* space left in output string */
unsigned temp;                  /* temps */
int wdaytemp;
...
/* Use a copy of the appropriate _lc_time_data pointer.  This
should prevent the necessity of locking/unlocking in mthread
