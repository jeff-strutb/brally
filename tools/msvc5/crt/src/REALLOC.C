            ...
            if ( newsize <= _HEAP_MAXREQ ) {
                ...
                if ( (pmap = __sbh_find_block(pblock, &preg, &ppage)) != NULL )
                {
                    ...
                    if ( newsize < __sbh_threshold ) {
                        if ( __sbh_resize_block(preg, ppage, pmap,
                             newsize >> _PARASHIFT) )
                        {
                            retp = pblock;
                        }
                        else if ( (retp = __sbh_alloc_block(newsize >>
                                  _PARASHIFT)) != NULL )
                        {
                            oldsize = ((size_t)(*pmap)) << _PARASHIFT ;
                            memcpy(retp, pblock, __min(oldsize, newsize));
                     ...
                     */
                    if ( (retp == NULL) &&
                         ((retp = HeapAlloc(_crtheap, 0, newsize)) != NULL) )
                    {
                        oldsize = ((size_t)(*pmap)) << _PARASHIFT;
                        memcpy(retp, pblock, __min(oldsize, newsize));
                        __sbh_free_block(preg, ppage, pmap);
