 ...
 *
 * cdecl, one arg, `ret`, 322 B. Twin of 0x1003A580 on the +0x24 time
 * slot instead of +0x20. Format a time into the owner\'s
...
typedef char chk_nameH[(unsigned)&((Item438H *)0)->szName == 9 ? 1 : -1];

class Obj3A6D0 {
public:
    char    pad000[0x2B5C];
    Item438H m2B5C;              /* +0x2B5C */
};

typedef char chk_itemH[(unsigned)&((Obj3A6D0 *)0)->m2B5C == 0x2B5C ? 1 : -1];

extern "C" {
float g_brTime5C24;             /* 0x10AC5C24 */
...
int BrItemSetBestTime_1003A6D0(Obj3A6D0 *pObj)
{
    char  szTime[32];
    float t = g_brTime5C24;
    char *pLabel;
