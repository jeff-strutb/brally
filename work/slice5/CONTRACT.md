...
## The packet headers ARE trustworthy this round (they were not last round)

Each block is headed:

    ; ===== WANTED AS: <name>   @ <address>   (needed by: <modules>) =====

The previous round's queue was built by a buggy extractor and about half the
listings showed the WRONG function under the right name. That is fixed: the
address now comes from same-declaration evidence only, the queue was validated
against ground truth that agents established independently, and the generator
self-checks every block (it omits a listing rather than print a mismatched one).
All 123 blocks verified paired before hand-out.

You should still sanity-check that the body's `@ ADDR` matches the header. If it
ever does not, STOP and report it -- that means the generator regressed.
