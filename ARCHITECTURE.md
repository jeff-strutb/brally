...
| `BRD3D.dll` | 581,632 | ~2,700 | the game, Direct3D build | no `glide2x`; the extra 100 KB is a statically linked CRT |
...
ownership could be attributed for only about 33 KB, and over 26 KB of that is
...
run time from "installed and did nothing", and that is exactly why "the menu
does not navigate" was investigated for weeks as a bug in the menu.
...
one slot to another, and it misses a pointer *computed* rather than taken, which
is exactly what the undecoded 256-entry table at 0x100A9A58 is likely to be.
`installs` is therefore a superset of "could install" and a subset of "does
install at run time".
...
The high opcodes (`0xB1` to `0xFC`) are the N64 F3DEX/RDP command set; the low
ones (`0x01` to `0x06`) are the F3DEX matrix and display-list commands. A
sibling analysis of the N64 ROM confirms the PC build handles a subset of stock
F3DEX and repurposes `0xE1`, which the N64 never emits.
...
through, consistent with the engine-wide pattern. The largest are 1,000 to
1,200 byte functions that reference **no strings at all** and open by reading
globals in the `0x104Bxxxx`/`0x104Cxxxx` range, several in exact-size twin pairs
(`0x100228F0` and `0x100231D0` are both 1,223 bytes). That shape (long,
string-free, globals-driven, duplicated per variant) is what a specialised
inner loop looks like, and it is why they have no Glide twin: the Glide build
reaches the hardware through `glide2x.dll` instead.
