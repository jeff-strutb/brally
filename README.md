...
**~30% of the game is decompiled.** That figure has a stated denominator so it
...
Glide `0x1001CC00`, 324 bytes. **It has never been ported.** Of the eleven
functions it calls, **nine are absent from this port**, including
`0x10019670` (187 B), which is almost certainly the main loop, and
`0x1001D8A0` (924 B), which parses the arguments everything else depends on.
